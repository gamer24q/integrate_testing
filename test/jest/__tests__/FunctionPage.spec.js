...
test('trigger click event on button ', async () => {
  ...
  expect(wrapper.vm.addition(1,2)).toBe(3) // เพิ่มฟังก์ชั้นการลบ คูณ และหาร
})
test('trigger click event on button ', async () => {
  const wrapper = shallowMount(FunctionPage)
  const button = wrapper.find('button')
  await button.trigger('click')
  expect(wrapper.vm.subtraction(1,2)).toBe(3) // เพิ่มฟังก์ชั้นการลบ คูณ และหาร
})
test('trigger click event on button ', async () => {
  const wrapper = shallowMount(FunctionPage)
  const button = wrapper.find('button')
  await button.trigger('click')
  expect(wrapper.vm.multiplication(1,2)).toBe(3) // เพิ่มฟังก์ชั้นการลบ คูณ และหาร
})
test('trigger click event on button ', async () => {
  const wrapper = shallowMount(FunctionPage)
  const button = wrapper.find('button')
  await button.trigger('click')
  expect(wrapper.vm.divide(1,2)).toBe(3) // เพิ่มฟังก์ชั้นการลบ คูณ และหาร
})